...
export default function EditImageModal({ image, onClose, onSuccess }: EditImageModalProps) {
  ...
  // Load categories and pre-select current ones
  useEffect(() => {
    fetch("/api/categories")
      ...
      .then(d => {
        ...
        setCategories(cats);
        // Pre-select categories that match image's current categories by name
        const preSelected = cats
          .filter(c => image.categories.includes(c.name))
          .map(c => c.id);
      ...
      <div className="edit-overlay" onClick={onClose}>
        <div className="edit-window" onClick={e => e.stopPropagation()}>
          ...
          <div className="edit-body">
            ...
            <div className="edit-row-inline">
              ...
              <div className="edit-row">
                ...
                <div className="visibility-row">
                  <label>
                    <input type="radio" name="edit-vis" value="public"
                      checked={visibility === "public"}
                      onChange={() => setVisibility("public")} />
                    público
                  </label>
                  <label>
                    <input type="radio" name="edit-vis" value="private"
            ...
            <div className="edit-row">
              <label className="xp-label">Descripción</label>
              <textarea
            ...
            <div className="edit-row">
              <label className="xp-label">Fuente / link original</label>
              <input
                className="xp-input"
                value={sourceUrl}
            ...
            <div className="edit-row">
              <label className="xp-label">Categorías</label>
              <div className="category-chips">
                {categories.length === 0
                  ? <span style={{ fontSize: "11px", color: "#888", padding: "2px 4px" }}>Cargando...</span>
                  : categories.map(cat => (
                    <button
                      key={cat.id}
          ...
          <div className="edit-footer">
            ...
            <div>
              {confirmDelete ? (
                <div className="delete-confirm">
                  ...
                  <button className="xp-btn" onClick={() => setConfirmDelete(false)}>No</button>
                </div>
              ) : (
                <button
                  className="xp-btn"
            ...
            <div className="edit-footer-right">
              <button className="xp-btn" onClick={onClose} disabled={saving}>Cancelar</button>
              <button
                className="xp-btn"
                onClick={handleSave}
                disabled={saving}
